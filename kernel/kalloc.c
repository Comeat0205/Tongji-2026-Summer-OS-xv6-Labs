...
struct {
  ...
  struct run *freelist;
  // reference counts for physical pages, indexed by pa / PGSIZE
...
freerange(void *pa_start, void *pa_end)
{
  ...
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE){
    // set ref to 1 so kfree() will put the page on the free list
...
krefcnt(void *pa)
{
  ...
  acquire(&kmem.lock);
  int c = kmem.refcnt[(uint64)pa / PGSIZE];
  release(&kmem.lock);
  return c;
}
